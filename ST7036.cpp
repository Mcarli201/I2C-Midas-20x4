...
const uint8_t CONTRAST_CMD   = 0x79; // Set contrast LCD command
const uint8_t FUNC_SET_TBL0  = 0x3a; // Function set - 8 bit, 2 line display 5x8, inst table 0
...
const uint8_t dram_dispAddr [][4] =
{
   { 0x00, 0x00, 0x00 },  // One line display address
   { 0x00, 0x14, 0x40, 0x54 },  // Two line display address
...
void ST7036::init () 
{
   ...
   Wire.write ( 0x3A );  // Set BIAS - 1/5
   delay (10);
   Wire.write ( 0x1C );  // Set BIAS - 1/5
   delay (10);
   Wire.write ( 0x39 );  // Set BIAS - 1/5
   delay (10);
   Wire.write ( 0x1c );  // Set BIAS - 1/5
   Wire.write ( 0x79 );  // Set contrast low byte
   Wire.write ( 0x5D );  // ICON disp on, Booster on, Contrast high byte 
